...
def main():
    ...
    repofile = "repo"
    repourl = "https://raw.githubusercontent.com/Admicos/cc-packman-repo/" \
              "master"  # flake8 please.
    ...
    pkg = {
           ...
           "size": None,
           "dependencies": input("Package Dependencies (split by spaces): "),
           "link": input("Package link (for README.md): ")
        }

    try:
        pkg["size"] = os.stat(pkg["name"] + "/" + pkg["name"] + ".lua").st_size
    except FileNotFoundError as fnfe:
        print("\nFile not found: " + pkg["name"] + "/" + pkg["name"] + ".lua")
